...
import { useAsyncReference, useScroll, useWidth } from '../customHooks'
...
export default function HistoryPage() {
    ...
    const [historyCaller, signal] = getCaller("");
    ...
    const requestAutoComplete = async ()=>{
        if(searchParams.current.keyword != null && searchParams.current.keyword.length > 3){
            const auto = await historyCaller(`/radio/api/autocomplete.php?limit=${10}&keyword=${searchParams.current.keyword}`); 
            if(auto != null) {
    ...
    const requestNumberOfTracks = async()=>{
        const num = await historyCaller("/radio/api/numberoftracksapi.php");
        if(num != null){
    ...
    const requestHistory = async (keyword: string, date: string, limit: number, offset: number)=>{
        removeScroll();
        let trackData;
        trackData = await historyCaller(`/radio/api/historyapi.php?limit=${limit}&offset=${offset}&keyword=${encodeURIComponent(keyword)}&date=${date}`);   
        if(trackData != null){
